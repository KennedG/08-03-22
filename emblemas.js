...
if(args0==='Verificado'){
  if(dindin<100) return message.reply(`Ops ${message.author} você não possui Cristais suficiente!`)
...
if(args0==='Coração'){
  ...
  if(args0==='Plug'){
    if(dindin<350) return message.reply(`Ops ${message.author} você não possui Cristais suficiente!`)
  
    message.reply('<:dcl_ypSino:914350019182723122> | Emblema setado com sucesso!')
    db.subtract(`cristais_${message.author.id}`, 350)
  
    db.set(`emblemas_${message.author.id}`, Plug)
  }

  if(args0==='Desenvolvedor'){
    if(dindin<400) return message.reply(`Ops ${message.author} você não possui Cristais suficiente!`)
  
    message.reply('<:dcl_ypSino:914350019182723122> | Emblema setado com sucesso!')
    db.subtract(`cristais_${message.author.id}`, 400)
...
> Nome: _Plug_
> Preço: 350 Cristais
Preview: <a:p_diorgifs:950876092393193482>

> Emblema #6
> Nome: _Desenvolvedor_
> Preço: 400 Cristais
Preview: <a:developer:912914634846109706>
